...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set webroot
WWW_DIR="/usr/share/webapps/sqlite"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
root_user_pass="${NGINX_ROOT_PASS_WORD:-$(__random_password)}" # root user password
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  __file_copy "/config/phpliteadmin/." "/etc/phpliteadmin/" |& tee -a "$LOG_DIR/init.txt" &>/dev/null
  # replace variables
  sed -i 's|REPLACE_BLOWFISH_SECRECT|'$blowfish_pass'|g' "/config/phpliteadmin/config.php"
  __replace "REPLACE_ADMIN_PASS_WORD" "$root_user_pass" "/config/phpliteadmin/config.php"
  ...
  __initialize_replace_variables "$WWW_DIR"
  # custom commands
  if [ ! -d "/config/phpliteadmin" ]; then
    mkdir -p "/config/phpliteadmin"
    __file_copy "/etc/phpliteadmin/." "/config/phpliteadmin/" |& tee -a "$LOG_DIR/init.txt" &>/dev/null
  fi
